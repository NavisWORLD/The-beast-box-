#!/usr/bin/env bash
# Explicit opt-in: run the native 12K server alongside the existing tiny bridge.
set -Eeuo pipefail
umask 077
...
for _ in range(120):
    try:
        ...
        if (info.get("ready") is True and info.get("model_id") == "rawrphos-native"
                and info.get("training_steps") == 12000
                and info.get("checkpoint_sha256") == "339fb8e1d6f3950e2aa15a6e33bf8c0f28dd655cefc93b7926fb7545e7e97601"):
            print("Verified RAWRPHOS native 12K on private loopback", flush=True)
            raise SystemExit(0)
