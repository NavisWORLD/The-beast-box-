#!/usr/bin/env bash
# Explicit opt-in: run the experimental, unpromoted native 18K server alongside the existing tiny bridge.
set -Eeuo pipefail
umask 077
...
for _ in range(120):
    try:
        ...
        if (info.get("ready") is True and info.get("model_id") == "rawrphos-native"
            ...
            print("Verified RAWRPHOS native EXPERIMENTAL 18K on private loopback", flush=True)
            raise SystemExit(0)
