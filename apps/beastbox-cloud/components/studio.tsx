...
import { Activity, ArrowDownToLine, ArrowLeftRight, ArrowRight, BrainCircuit, Check, ChevronDown, CircleHelp, CloudOff, Command, Database, File, FileText, Fingerprint, Github, Image as ImageIcon, LockKeyhole, LogOut, Menu, MessageCircle, Paperclip, Plus, Send, Settings2, Shield, ShieldCheck, Sparkles, Telescope, Trash2, X, Zap } from 'lucide-react';
...
function Login({configured,onLogin}:{configured:boolean;onLogin:()=>void}){
 ...
 <span className="login-emblem">✺</span><span className="eyebrow">OWNER ACCESS / PRIVATE</span><h1>Welcome back,<br/><em>cosmic traveler.</em></h1><p>{configured?'Your Beast Box is protected. Enter the owner password to open your workstation.':'Preview is not provisioned yet. Set BEASTBOX_OWNER_PASSWORD and BEASTBOX_CLOUD_AUTH_SECRET in Vercel Preview settings.'}</p>
...
export default function Studio({initialOwner,configured,initialBridge}:{initialOwner:boolean;configured:boolean;initialBridge:boolean}){
 const [owner,setOwner]=useState(initialOwner),[bridge,setBridge]=useState(initialBridge),[page,setPage]=useState<Page>('BRAIN');
 ...
 const load=useCallback(async()=>{
   ...
   try{
    ...
    const available=status.bridgeConfigured===true;setBridge(available);
    if(!available){setModel('NOT CONNECTED');return;}
    const [conv,orb,mem,tr,prof,storage]=await Promise.all(['conversation','orbit','memory','trace','provider','storage'].map(x=>api('bridge/'+x)));
    setTurns(Array.isArray(conv.turns)?conv.turns.map((entry:unknown,i:number)=>{
      ...
      return {id:String(e.id||i),role,text:String(e.text||e.content||''),kind:role};
    }):[]);
   ...
   }catch(e){setError((e as Error).message);setBridge(false);setModel('UNAVAILABLE');}
 },[owner]);
 ...
 async function logOut(){try{await api('session',{method:'DELETE'});}finally{setOwner(false);setTurns([]);setAttachments([]);}}
 if(!owner)return <Login configured={configured} onLogin={()=>setOwner(true)}/>;
 const connected=bridge&&model!=='UNAVAILABLE';
 ...
 <div className="main-shell"><header className="app-header"><div className="header-left"><button className="icon-button mobile-only" onClick={()=>setMenu(true)} aria-label="Open navigation"><Menu size={22}/></button><span className="tiny-orbit">✺</span><span className="breadcrumbs">YOUR UNIVERSE <b>/</b> <strong>{page}</strong></span></div><div className="header-right"><span className={'status-chip '+(connected?'online':'offline')}><span className="pulse"/>{connected?'BRIDGE CONFIGURED':'BACKEND OFFLINE'}</span><button className="icon-button" aria-label="Refresh status" onClick={()=>void load()}><Activity size={17}/></button><span className="avatar">CD</span></div></header>
