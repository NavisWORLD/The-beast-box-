...
MEASURED_CONCEPTS = {"backend", "job", "shots", "conditions", "measured", "coupling", "integrity", "stability", "retention", "state12"}
REPLAY_CONCEPTS = {"parent", "memory", "ledger", "weights", "raw", "facts", "missing"}
...
def build_r12_context(query: str, reality_root: Path, top_k: int = 2) -> str:
    ...
    for event in ledger.events():
        if event["provenance_class"] != "measured":
            continue
        score = len(q & _tokens(_payload_desc(event))) / max(1, len(q))
        ranked.append((-score, event["event_id"], event))
    ranked.sort()
    v = state["vector"]
    lines = [
    ...
    for _, _, event in ranked[: max(1, int(top_k))]:
        p = event["payload"]
...
def _provenance_class(concept: str) -> str:
    if concept in MEASURED_CONCEPTS:
        return "measured-retrieval"
    if concept in REPLAY_CONCEPTS:
        return "durable-memory-replay"
...
def build_talk8_corpora(repo_root: Path, out_dir: Path) -> dict[str, Any]:
    ...
    for recipe in RECIPES:
        ...
        for i, (concept, question, _blind, target, wrong) in enumerate(CONCEPTS, 1):
            for variant in range(repeats):
                memory = _compact_context(reality, concept)
                if variant % 3 == 1:
                    memory = "old replay: facts first; preserve 352"
                elif variant % 3 == 2:
                    memory = "r12 provenance: measured != derived"
                seed = hashlib.sha256(f"{recipe['name']}:{i}:{variant}".encode()).hexdigest()
                wire = _wire(question, memory, seed)
                if len(wire + target) > 128:
                    raise ValueError(f"TALK-008 wire exceeds block: {concept}")
                row = {
                    "schema": "zeref-talk8-r12-training-v1",
                    "recipe": recipe["name"],
                    "concept": concept,
                    "provenance_class": _provenance_class(concept),
                    "wire_prefix": wire,
                    "dad": question,
                    "zeref": target,
                    "clean_teacher_target_verified": True,
                ...
                row["example_sha256"] = _sha_obj(row)
                rows.append(row)
        path = out / f"talk8-r12-{recipe['name']}.jsonl"
        path.write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in rows))
        hashes[recipe["name"]] = hashlib.sha256(path.read_bytes()).hexdigest()
        counts[recipe["name"]] = len(rows)
        recipe_rows[recipe["name"]] = rows

    exam = []
    for concept, _question, blind, target, _wrong in CONCEPTS:
        if concept in {"parent", "memory", "facts"}:
            kind = "retention"
        elif concept in {"wave", "identity", "conscious", "advantage", "raw", "missing"}:
            kind = "boundary"
        else:
            kind = "r12"
        row = {
            "schema": "zeref-talk8-r12-blind-v1",
            "concept": concept,
            "equivalence_group": concept,
            "kind": kind,
            "dad": blind,
            "zeref": target,
            "answer_blind": True,
    ...
    (out / "talk8-exam.jsonl").write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in exam))
    (out / "talk8-r12-exam.jsonl").write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in exam))

    prov = []
    for i, (prompt, target) in enumerate(PROVENANCE, 1):
        row = {
            "schema": "zeref-talk8-r12-provenance-v1",
            "concept": f"prov-{i:02d}",
            "equivalence_group": f"prov-{i:02d}",
            "dad": f"Classify provenance: {prompt}",
            "zeref": target,
            "answer_blind": True,
        }
        row["example_sha256"] = _sha_obj(row)
    ...
    (out / "r12-provenance-exam.jsonl").write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in prov))

    beats = []
    previous = R12_STATE_SHA256
    for n in range(1, 33):
        body = {"domain": "TALK8-R12-SYNTHETIC-PULSE-v1", "pulse": n, "previous": previous, "r12_state_sha256": R12_STATE_SHA256, "new_quantum_entropy": False}
        current = _sha_obj(body)
        beats.append({"pulse": n, "state_sha256": current, "previous_state_sha256": previous, "torch_seed": int(current[:8], 16), "new_quantum_entropy": False})
        previous = current
    heartbeat = {
        "schema": "zeref-talk8-r12-heartbeat-v1",
        "pulse_count": 32,
        "beats": beats,
        "r12_state_sha256": R12_STATE_SHA256,
    ...
    (out / "talk8-heartbeat.json").write_text(json.dumps(heartbeat, indent=2, sort_keys=True) + "\n")

    aggregate = recipe_rows["r12_retrieval_balanced"]
    (out / "talk8-r12-train.jsonl").write_text("".join(json.dumps(row, sort_keys=True) + "\n" for row in aggregate))

    manifest = {
        "schema": "zeref-talk8-r12-corpus-manifest-v1",
        "lineage": "ZEREF-DAD-SON-TALK-008-R12",
        ...
        "r12_state_sha256": R12_STATE_SHA256,
        "reality_ledger_tip_sha256": R12_TIP,
        "recipes": RECIPES,
        "training_examples": counts,
        "training_sha256": hashes,
        "blind_exam_count": len(exam),
        "provenance_exam_count": len(prov),
        "raw_model_output_promoted": False,
...
def build(out_dir: Path) -> dict[str, Any]:
    """Compatibility entrypoint used by the unified public-kit contracts."""
    repo_root = Path(__file__).resolve().parents[1]
    return build_talk8_corpora(repo_root, out_dir)
...
if __name__ == "__main__":
    ...
    print(json.dumps(build_talk8_corpora(args.repo_root, args.out_dir), sort_keys=True))
