...
def _refresh_checksums(run_dir: Path) -> None:
    names = [line.split("  ", 1)[1] for line in (run_dir / "CHECKSUMS.sha256").read_text(encoding="utf-8").splitlines() if line]
    rows = [f"{_sha256_file(run_dir / name)}  {name}" for name in names]
    (run_dir / "CHECKSUMS.sha256").write_text("\n".join(rows) + "\n", encoding="utf-8")


def _migration_fixture() -> tuple[TinySparkCST, dict, dict]:
    torch.manual_seed(20260913)
    parent = TinySparkCST(vocab=7, block=16, d_model=24, n_layers=1)
    config = {"vocab": 7, "block": 16, "n_layer": 1, "n_head": 4, "n_embd": 24, "d54": 54}
    tokenizer = {chr(ord("a") + index): index for index in range(7)}
...
def _build_corpora(tmp_path: Path):
    lexical_records = [
        {"lemma": f"term{i}", "part_of_speech": "noun", "definition": f"plain definition {i}", "synonyms": [], "antonyms": [], "examples": [f"term{i} example"], "source_id": "lex"}
        for i in range(24)
    ]
    world_records = [{"title": f"topic{i}", "text": f"plain world fact {i}", "source_id": "world"} for i in range(24)]
    lexical_root = tmp_path / "lexical"
    world_root = tmp_path / "world"
    lexical = build_corpus(kind="lexical", records=lexical_records, sources=[{"source_id": "lex", "uri": "local://lex", "license": "CC0-1.0"}], output_dir=lexical_root)
    world = build_corpus(kind="world", records=world_records, sources=[{"source_id": "world", "uri": "local://world", "license": "CC0-1.0"}], output_dir=world_root)
    return lexical, lexical_root, world, world_root


def _quantum_receipt() -> dict:
    metadata = {
        "source": "ibm-quantum", "mode": "REAL_IBM", "result_kind": "observed-counts",
        "native_job_id": "job-hardening-test", "backend": "ibm_test_backend", "shots": 128,
        "counts": {"00": 64, "01": 32, "10": 16, "11": 16}, "circuit_sha256": "c" * 64,
...
def _world_fixture(tmp_path: Path) -> dict:
    ...
    training_text = (lexical_root / "train.txt").read_text(encoding="utf-8") + (world_root / "train.txt").read_text(encoding="utf-8")
    ...
    return {
        "parent": parent,
        "parent_config": {"vocab": len(chars), "block": 32, "n_layer": 2, "n_head": 4, "n_embd": 24, "d54": 54},
        "tokenizer": tokenizer,
        "lexical": lexical, "lexical_root": lexical_root, "world": world, "world_root": world_root,
        "quantum": _quantum_receipt(),
        "train_config": {"schema": "zeref-phos-world-train-config-v1", "steps": 2, "seq": 16, "batch": 2, "lr": 0.001, "seed": 314159, "sampling_weights": {"lexical": 1, "world": 1}},
    }


def _run(fixture: dict, output_dir: Path) -> None:
    train_descendant_generation(
        parent_model=fixture["parent"], parent_config=fixture["parent_config"], tokenizer=fixture["tokenizer"],
        parent_checkpoint_sha256="a" * 64, parent_architecture_sha256="b" * 64,
        expected_parent_parameter_sha256=parameter_sha256(fixture["parent"]),
        lexical_manifest=fixture["lexical"], lexical_root=fixture["lexical_root"],
        world_manifest=fixture["world"], world_root=fixture["world_root"], quantum_receipt=fixture["quantum"],
        config=fixture["train_config"], output_dir=output_dir, generation_id="zeref-phos-hardening-g001",
    )
...
def test_corpus_verifier_rejects_semantic_tamper_even_when_artifact_hash_is_refreshed(tmp_path: Path):
    ...
    train_jsonl.write_text("".join(json.dumps(row, sort_keys=True, separators=(",", ":")) + "\n" for row in rows), encoding="utf-8")
    forged["artifacts"]["train.jsonl"] = _sha256_file(train_jsonl)
