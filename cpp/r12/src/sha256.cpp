...
std::string sha256_bytes(const std::vector<std::uint8_t>& input){
    ...
    for(std::size_t off=0;off<data.size();off+=64){
        std::uint32_t w[64]{};
        for(int i=0;i<16;++i){std::size_t j=off+i*4;w[i]=(std::uint32_t(data[j])<<24)|(std::uint32_t(data[j+1])<<16)|(std::uint32_t(data[j+2])<<8)|data[j+3];}
...
std::string sha256_text(const std::string& text){return sha256_bytes(std::vector<std::uint8_t>(text.begin(),text.end()));}
std::string sha256_file(const std::string& path){std::ifstream in(path,std::ios::binary);if(!in)throw std::runtime_error("cannot open file: "+path);std::vector<std::uint8_t> data((std::istreambuf_iterator<char>(in)),{});return sha256_bytes(data);}
}
