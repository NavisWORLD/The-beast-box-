...
def test_run023_live_workflow_is_goal_directed_model_logit_transport() -> None:
    ...
    assert "Prove model-logit decoder prompts fit native 128-token window" in workflow
    assert "/tokenize" in workflow
    assert "build_tool_choice_request" in workflow
    assert "build_argument_request" in workflow
    assert "max_tokens=24" in workflow
    assert "--max-tokens 24" in workflow
    assert '"compact_action_transport": "model-logit-tool-plus-generated-argument"' in workflow
    ...
    assert "ZEREF_ACTION_PREFLIGHT=PASS count=2 context=128 transport=logit-decoder objective=boundary" in workflow
