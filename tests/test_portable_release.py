...
def artifacts(tmp_path):
    ...
    for platform in ('ubuntu-24.04', 'windows-latest'):
        d = assets / f'beast-desktop-{platform}-0.6.0'
    ...
    d = assets / 'beast-android-0.6.0/dist'
    ...
    write(assets / 'beast-ios-evidence-0.6.0/acceptance.json', dict(passed=True, process_launches=3, models=['A', 'B', 'A']))
    for flavor in ('simulator', 'unsigned-device'):
        p = assets / f'beast-ios-{flavor}-0.6.0' / f'{flavor}.zip'
...
def test_refuses_broken_platform_chain(artifacts, failure):
    assets, output = artifacts
    if failure == 'apk':
        (assets / 'beast-android-0.6.0/dist/test.apk').write_bytes(b'corrupt')
    elif failure == 'restart':
        write(assets / 'beast-desktop-ubuntu-24.04-0.6.0/restart.json', dict(valid=True, system_id='different', turn_before=1))
    elif failure == 'missing':
        (assets / 'beast-ios-evidence-0.6.0/acceptance.json').unlink()
