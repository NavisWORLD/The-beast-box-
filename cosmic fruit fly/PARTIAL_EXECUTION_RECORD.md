...
## Hard-mode three-token continuation (2026-09-20)

A further isolated local continuation adds an experimenter-defined three-token compositional code (8 camera-detectable object categories × 3 gesture glyphs × 3 intent glyphs), including two *few-shot* new object categories. Of 72 possible combinations, 48 receive supervised examples and 24 are held out. The withheld camera is partially occluded (20–28% of sprite foreground pixels) with 9 distracting speckles; testing never provides labels. The grammar and RGB calibration are fixed in code, and the source-derived 42-node neural state does **not** drive the factorized decoder. An authorized correct three-token message unlocks an **in-memory simulated resource**; wrong or unauthorized output does not. Later all token mappings reverse and are retrained; a simulated software offspring may copy learned mapping dictionaries. No biological language, zero-shot category recognition, genetic inheritance or physical-world authority is claimed.

**Executed:** four seeds × ten matched arms × 264 trials = 10,560 episodes. Primary source-derived topology + historic nine-IBM-record replay: 192/192 retained pre-reversal, 96/96 held-out noisy, 96/96 reversed held-out noisy, 96/96 inherited-offspring held-out noisy. Original replay off, shuffled replay, rewired topology and no neural propagation have identical scores. Holistic phrase memory fails all 96/96 held-out cases (0 correct); frozen reversal fails post-flip; blank offspring fails transfer; typing denied unlocks no resource. Seed-0 modeled neural-state on/off delta: 0.01708587, no task-performance difference. **There is no observed advantage attributable to quantum replay or anatomical topology.** These deliberately simplified supervised tasks do not establish open-ended language.

**Verification:** an initial isolated suite had one missing historical-video-fixture failure; after restoring the verified historical fixture without changing it, 63 tests passed and three skipped. Independent complete four-seed rerun reproduced both `runs.jsonl` and `results.json` byte-for-byte. Ledger SHA-256: `4b8bf249a4559df6c55ecd6b9205fc847f737660ecb4e31536b206b29ad36666`. H.264 1280×720 rendered replay: 7:28 (448 sec), 1792 frames at 4 fps, SHA-256 `c93490433604a813f5b7d150eb28f3afdbd17094774a95e43b420cf08078f6d7`. 112 evenly spaced seed-0 trials are shown; every frame reconstructs and hash-checks the corresponding virtual camera. Extracted final local ZIP tested: 63 passed, three skipped; video hash matched its manifest. Local ZIP SHA-256: `d4e313e476729906d65b0416187d31eb3e2df13eae3c0fa15f2e6d674b12256b`.

**Sync status:** THIS IS A DOCUMENTATION-ONLY GITHUB COMMIT. New runnable code, source data, evidence ZIP, and video are exported locally for the user; they are **not committed to this GitHub branch**. Do not merge. Open gates include actual source sync and CI, FlyWire CC BY-NC 4.0 noncommercial data licensing, production interfaces, proper signal validation, and more complete quantum archive research. Preserve all previous nulls and failures.
