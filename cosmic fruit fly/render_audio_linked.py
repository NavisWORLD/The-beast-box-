"""Polished pixel-art video of a verified executed audio-to-QPU replay trace.
...
from audio_linked_fusion import OUT, PHASES
...
def sources(out=OUT):
    ...
    if sha256(out/'runs.jsonl')!=r['ledger_sha256'] or sha256(DATA)!=r['source_file_sha256']['flywire_subset']:
        raise RuntimeError('source or ledger hash mismatch')
    rows=[json.loads(line) for line in (out/'runs.jsonl').read_text().splitlines()]
    a=next(v for v in rows if v['seed']==0 and v['arm']=='audio_linked_qpu')
...
def render_frame(trial,shot,tick,sub,frame_no,world,data,layout):
    ...
    text(d,(35,23),'COSMIC FRUIT FLY  /  AUDIO x IBM QPU',18,CYAN)
    ...
    text(d,(633,578),'CST/dyn12  +  ARCHIVED SOURCE REPLAY',13,CYAN)
    ...
    text(d,(858,596),'AUDIO PACKET / 20 WINDOWS',10,PINK)
    text(d,(858,615),f'SEG {str(shot["audio_segment"]):>2}   DRIVE {shot["audio"]:+.5f}',11,WHITE)
    text(d,(858,638),'IBM MARRAKESH / ARCHIVED',10,YELLOW)
    text(d,(858,657),f'QPU {shot["quantum"]:+.5f}  {shot["qpu_bits_histogram_surrogate"] or "OFF"}',11,WHITE)
    text(d,(858,677),'AUDIO != VERIFIED PHYSIOLOGY',10,PINK)
    text(d,(858,693),'4096 COUNTS / ARTIFICIAL ORDER',10,MUTED)
    ...
    if trial['phase']=='offspring_transfer_noise' and trial['trial']>=160:
        ...
        text(d,(36,191),'MEASURED ABLATIONS / 8 SEEDS / 10,752 TRIALS',14,CYAN)
        text(d,(37,223),'BASELINE '+score('baseline')+'    OLD IBM '+score('nine_summary'),13,WHITE)
        text(d,(37,249),'AUDIO '+score('audio_features')+'    AUDIO+QPU '+score('audio_linked_qpu'),13,WHITE)
        text(d,(37,277),'REAL BIO NOT EXECUTED  /  NO DEMONSTRATED ADVANTAGE',11,YELLOW)
    return im
...
def render(out=OUT,destination=None,preview=False):
    ...
    manifest={'file':dest.name,'frames':count,'fps':FPS,'duration_seconds':count/FPS,
        ...
        'selection':'126 deterministic evenly spaced trials across five phases of seed-0 audio-linked-QPU run',
        'camera_validation':'each image regenerated from actual virtual camera and sha256 checked',
        'boundaries':'memorial-audio-derived features; IBM histogram surrogate, simulated neural dynamics; NOT physiology or live QPU'}
