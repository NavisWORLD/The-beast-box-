"""Install only the pinned public 12K release into an empty private model directory."""
import argparse
import hashlib
import json
import os
...
TAG = "rawrphos-native-step-00012000-run-35819774878"
BASE = "https://github.com/NavisWORLD/The-beast-box-/releases/download/" + TAG
ARCHIVE_SHA = "c956ba605f1194587c73f471103d97d86c496b89f943380fb480862f4df60613"
...
def verify(path):
    ...
    if metadata["training_steps"] != 12000 or metadata["model_id"] != "rawrphos-native":
...
def install(destination):
    ...
    with tempfile.TemporaryDirectory(prefix=".rawrphos-install-", dir=destination.parent) as temp:
        ...
        size = 0
        with urllib.request.urlopen(BASE + "/rawrphos-native-step-00012000.tar.gz", timeout=180) as response, archive_file.open("wb") as output:
        ...
        with tarfile.open(archive_file, mode="r:gz") as archive:
            for member in archive.getmembers():
                ...
                if (name.is_absolute() or ".." in name.parts or not name.parts or
                        name.parts[0] != "step-00012000" or
                        not (member.isfile() or member.isdir())):
                    raise ValueError("unsafe release archive member")
            archive.extractall(staging, filter="data")
        checkpoint = staging / "step-00012000"
...
def main():
    ...
    print(json.dumps({"model_id": "rawrphos-native", "training_steps": 12000,
