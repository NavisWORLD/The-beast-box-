...
def create_replay(run: Path):
    ...
    html = HTML.replace('ACTUAL LEDGER VIEW ·', 'REPLAY OF RECORDED EVENTS ·')
    # Bundle only public, hash-verified events. Remove network polling and endpoints.
    html = html.replace("refresh();setInterval(refresh,1500);", "events=JSON.parse(document.getElementById('bundled').textContent);index=Math.max(0,Math.min(events.length-1,Number(new URLSearchParams(location.search).get('event') ?? events.length-1)));render();")
