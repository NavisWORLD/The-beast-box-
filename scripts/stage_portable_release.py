...
def stage(assets: Path, output: Path, source: str, version: str = "0.7.0") -> None:
    ...
    for platform in ('ubuntu-24.04', 'windows-latest'):
        ...
        archive(directory, output / f'beast-desktop-{platform}-{version}.zip', executable=True)
        results[platform] = 'PASS: executable, restart, native clients, checked installer'
    android = assets / f'beast-android-{version}'
    ...
    results['android'] = 'PASS: API 35 x86_64 emulator, embedded runtime A/B/A and restart; signing from env or debug'
    ios = assets / f'beast-ios-evidence-{version}'
    ...
    for flavor in ('simulator', 'unsigned-device'):
        matches = list((assets / f'beast-ios-{flavor}-{version}').glob('*.zip'))
    ...
    (output / 'PORTABLE_VERIFICATION.json').write_text(json.dumps({
        ...
        'limitations': ['Mobile fixture providers are deterministic test components, not trained models',
                        ...
                        'Android release signing is env-supplied; debug signing is used when the keystore is absent; no Play production distribution',
